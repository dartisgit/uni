...
pub use auth::{AuthorizedKey, InMemoryKeyStore, KeyStore};
...
async fn main() -> anyhow::Result<()> {
    ...
    if config.ssh.enabled {
        ...
        // No persistence layer exists yet, so this starts empty: no key is
        // authorized until one is added. Once `unicorn-core` grows real
        // storage, replace this block with a `KeyStore` impl that loads
        // `unicorn_core::models::SshKey` records from the database instead.
        //
        // To seed a key for local testing right now, add it before
        // spawning the server, e.g.:
        //
        //   let store = InMemoryKeyStore::new();
        //   store.add_key(unicorn_ssh::AuthorizedKey {
        //       fingerprint: "SHA256:...".to_string(),  // `ssh-keygen -lf ~/.ssh/id_ed25519.pub`
        //       owner_username: "neo".to_string(),
        //   });
        //   let key_store: Arc<dyn unicorn_ssh::KeyStore> = Arc::new(store);
        let key_store: Arc<dyn unicorn_ssh::KeyStore> = Arc::new(InMemoryKeyStore::new());
