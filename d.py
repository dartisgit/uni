...
authors.workspace = true
license.workspace = true
description = "Rust-native SSH server for Unicorn, built on russh, with real public-key authentication against a pluggable KeyStore."
...
thiserror.workspace = true
rand = "0.8"
# async-trait is required because `KeyStore` is used as `Arc<dyn KeyStore>`
# (dynamic dispatch across every SSH connection handler), and native
# async-fn-in-traits is not yet dyn-compatible in stable Rust.
...
FILES["crates/unicorn-ssh/src/auth.rs"] = r"""//! Public-key lookup for SSH authentication.
//!
...
FILES["crates/unicorn-db/Cargo.toml"] = r"""[package]
name = "unicorn-db"
version.workspace = true
edition.workspace = true
authors.workspace = true
license.workspace = true
description = "Postgres persistence for Unicorn: connection pool, migrations, and repository-pattern queries over unicorn-core's domain models."

[dependencies]
sqlx = { workspace = true, features = ["runtime-tokio", "tls-rustls", "postgres", "macros", "migrate", "chrono", "uuid"] }
tokio.workspace = true
tracing.workspace = true
thiserror.workspace = true
chrono.workspace = true
...
FILES["crates/unicorn-db/src/lib.rs"] = r"""//! Postgres persistence for Unicorn.
//!
...
FILES["crates/unicorn-db/src/error.rs"] = r"""use thiserror::Error;
...
FILES["crates/unicorn-db/src/ssh_keys.rs"] = r"""//! Postgres-backed implementation of `unicorn_ssh::KeyStore`.
//!
...
FILES["crates/unicorn-db/migrations/0001_initial.sql"] = r"""-- Initial schema for Unicorn's persistence layer.
--
...
authors.workspace = true
license.workspace = true
description = "The `unicorn` binary: wires config, logging, the database, the SSH server, and the TUI together."
...
unicorn-core = { path = "../unicorn-core" }
unicorn-ssh = { path = "../unicorn-ssh" }
unicorn-db = { path = "../unicorn-db" }
unicorn-tui = { path = "../unicorn-tui" }
"""

FILES["crates/unicorn-cli/src/main.rs"] = r"""//! The `unicorn` binary. Loads configuration, starts logging, connects to
