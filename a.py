...
# NOTE: the `Result<Auth, Self::Error>` / `Result<bool, Self::Error>` return
# types above shadow the crate-local `Result<T> = Result<T, SshError>`
# alias; that's deliberate (the trait requires `std::result::Result<_,
# Self::Error>` with `Self::Error = russh::Error`, distinct from this
# crate's own `SshError`), but is flagged here in the generator itself as a
# second thing worth a close look during `cargo check -p unicorn-ssh`.
...
def main() -> None:
    ...
    print(
        ...
        "\n"
        "If unicorn-ssh fails to check, see its module docs (crates/unicorn-ssh/src/lib.rs) -\n"
        "it's flagged as the one crate here most likely to need a version-specific fix.\n"
    )
